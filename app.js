...
function setWarnings(list) {
  if (!list || !list.length) {
    ...
    $('warnings').textContent = 'Warnings: ' + list.join('; ');
  }
}

function renderTable(el, rows) {
  if (!rows || !rows.length) {
    el.innerHTML = '<thead><tr><th>Empty</th></tr></thead>';
...
function loadText(text, sourceName) {
  ...
  current.nodes = res.nodes;
  current.edges = res.edges;
  ...
  if ((current.nodes.length + current.edges.length) > 10000) {
    current.graph.setPhysics(false);
...
function initControls() {
  ...
  $('searchBtn').addEventListener('click', () => {
    ...
    if (current.graph) current.graph.selectBy(q);
  });
