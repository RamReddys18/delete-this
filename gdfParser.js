function parseDelimitedLine(line,delim){
  const out=[];let i=0;let cur="";let inQ=false;while(i<line.length){
    const c=line[i];
    if(inQ){
...
function coerce(val,type){
  ...
  if(type==='int'){const n=parseInt(val,10);return isNaN(n)?undefined:n}
  if(type==='float'){const n=parseFloat(val);return isNaN(n)?undefined:n}
  if(type==='bool'){const v=val.trim().toLowerCase();return v==='true'||v==='1'}
  return String(val)
}
...
function parseGDF(text){
  ...
  if(lines.length===0)return {nodes:[],edges:[],warnings:["Empty file"]};
  lines[0]=stripBOM(lines[0]);
  let i=0;let nodeDef=null;let edgeDef=null;const nodes=[];const edges=[];
  while(i<lines.length){
    ...
    if(line.toLowerCase().startsWith('nodedef>')){
      ...
      while(i<lines.length){
        ...
        if(id===undefined){warnings.push('Node without id at line '+(i+1))}else{
          const label=obj.label!==undefined?String(obj.label):String(id);
          const attrs={};
          for(const c of nodeDef.cols){if(c.name!=='name'&&c.name!=='id'&&c.name!=='label'){attrs[c.name]=obj[c.name]}}
          nodes.push({id:String(id),label,attrs})
        }
        i++
      }
      continue
    ...
    if(line.toLowerCase().startsWith('edgedef>')){
      ...
      while(i<lines.length){
        ...
        if(from===undefined||to===undefined){warnings.push('Edge missing endpoints at line '+(i+1))}else{
          const w=obj.weight!==undefined?obj.weight:undefined;
          const attrs={};
          for(const c of edgeDef.cols){if(!['node1','node2','source','target','from','to','weight','label'].includes(c.name)){attrs[c.name]=obj[c.name]}}
          const label=obj.label!==undefined?String(obj.label):(w!==undefined?String(w):undefined);
          edges.push({from:String(from),to:String(to),label,weight:w,attrs})
        }
        i++
      }
      continue
  ...
  if(nodes.length===0&&edges.length===0){
    ...
    const inferredEdges=[];const nodeSet=new Set();
    for(const l of lines){
      ...
      if(from!==undefined&&to!==undefined){
        nodeSet.add(String(from));nodeSet.add(String(to));
        const w=vals.length>2?coerce(vals[2],'float'):undefined;
        inferredEdges.push({from:String(from),to:String(to),label:w!==undefined?String(w):undefined,weight:w,attrs:{}})
      }
    }
    if(inferredEdges.length>0){
      const inferredNodes=Array.from(nodeSet).map(id=>({id:String(id),label:String(id),attrs:{}}));
      warnings.push('No headers found; inferred graph from edge pairs using delimiter "'+(delim==="\t"?"TAB":delim)+'".');
      return {nodes:inferredNodes,edges:inferredEdges,warnings}
    }
    warnings.push('No nodedef>/edgedef> sections found or wrong delimiter (comma/semicolon/tab).')
  }
  return {nodes,edges,warnings}
}
window.GDFParser={parseGDF}